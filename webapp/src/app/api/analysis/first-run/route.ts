...
import { accessTokenFromRow, yt, isoDurationToSeconds, median } from "@/lib/server/youtube";
...
/** First mechanical read of a connected channel: last ~30 uploads, snapshots,
    and per-format baselines (Shorts vs long-form, like the local toolkit).
    The judgment layer (retention reads, packaging picks) builds on top of this. */
export async function POST(req: Request) {
  ...
  const { data: tokenRow } = await svc
    ...
    .select("id, refresh_token_ciphertext, authorized_channel_id")
    .eq("user_id", user.id)
    .is("revoked_at", null)
    .maybeSingle();
  ...
  try {
    ...
    if (!ch) return NextResponse.json({ error: "Couldn't read the channel." }, { status: 502 });
    ...
    // Upsert videos, keep internal ids for snapshots.
    ...
    const baselines: Record<string, number | null> = { longform: null, shorts: null };
    ...
    return NextResponse.json({
      ...
      longform_median: baselines.longform,
      shorts_median: baselines.shorts,
    });
  } catch (e) {
    return NextResponse.json(
