...
/** The sign-in flow, embedded in the settings page's Account section.
    It never navigates — completing any step fires an auth event and the
    page unlocks in place. */
export function AuthCard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (aal && aal.nextLevel === "aal2" && aal.currentLevel !== "aal2") setStep("mfa");
    })();
  ...
  async function verifyCode(e: React.FormEvent) {
    ...
    // No MFA pending: the auth event unlocks the page around this card.
  }
  ...
  async function verifyMfa(e: React.FormEvent) {
    ...
    try {
      ...
      if (vErr) throw vErr;
      // Verified: the auth event unlocks the page.
    } catch (err) {
  ...
  return (
    <div style={{ maxWidth: 430 }}>
      {step === "start" && (
        <>
          <p style={{ color: "var(--ink2)", fontSize: 13, margin: "0 0 14px" }}>
            Sign in with the Google account you use for YouTube, or with your email.
            This step only tells us who you are — it doesn&apos;t touch your channel.
          </p>
      ...
      {step === "code" && (
        <>
          <p style={{ color: "var(--ink2)", fontSize: 13, margin: "0 0 14px" }}>
            Enter the 6-digit code from your email. It expires in about an hour.
          </p>
          {note && <div className="ok-note">{note}</div>}
          {error && <div className="err">{error}</div>}
      ...
      {step === "mfa" && (
        <>
          <p style={{ color: "var(--ink2)", fontSize: 13, margin: "0 0 14px" }}>
            This account is protected with an authenticator app. Enter the current 6-digit code.
          </p>
          {error && <div className="err">{error}</div>}
